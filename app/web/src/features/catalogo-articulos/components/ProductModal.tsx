      ...
      <div className="flex min-h-screen items-end justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="relative inline-block w-full max-w-2xl my-8 overflow-hidden rounded-lg bg-white text-left align-middle shadow-xl transition-all transform">
          ...
          <form onSubmit={handleSubmit} className="px-6 py-4 space-y-6 max-h-[calc(100vh-200px)] overflow-y-auto">
            ...
            {/* ✅ NUEVA SECCIÓN: Stock Mínimo y Stock Máximo (Alertas) */}
            {trabajaConStock && (
              <div className="border-2 border-amber-200 bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl p-5 shadow-sm">
                ...
                <p className="text-xs text-amber-700 mb-4">
                  Configura los umbrales para recibir alertas cuando el stock esté bajo o en exceso
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setFormData(prev => ({ ...prev, stockMinimo: parseInt(e.target.value) || undefined }))}
                      className="w-full rounded-lg border-2 border-amber-300 bg-white px-4 py-2 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all"
                      placeholder="10"
                      min="0"
                    />
                  ...
                  {/* Stock Máximo */}
                  <div>
                    <label htmlFor="stockMaximo" className="block text-sm font-medium text-gray-700 mb-1">
                    ...
                    <input
                      ...
                      onChange={(e) => setFormData(prev => ({ ...prev, stockMaximo: parseInt(e.target.value) || undefined }))}
                      className="w-full rounded-lg border-2 border-gray-300 bg-white px-4 py-2 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all"
                      placeholder="Sin límite"
                      min="0"
                    />
                    <p className="text-xs text-gray-600 mt-1">
                      Recibirás una alerta si el stock supera este valor (sobrestock)
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-4 p-3 bg-amber-100 border border-amber-300 rounded-lg">
                  <div className="flex items-start space-x-2">
                    ...
                    <div className="text-xs text-amber-800">
                      <p className="font-semibold">Niveles de alerta:</p>
                      <ul className="mt-1 space-y-0.5">
                        <li>🔴 <strong>CRÍTICO:</strong> Stock = 0 o ≤ {((formData.stockMinimo ?? 10) * 0.5).toFixed(0)} unidades</li>
                        <li>🟡 <strong>BAJO:</strong> Stock ≤ {formData.stockMinimo ?? 10} unidades</li>
                        <li>🟢 <strong>NORMAL:</strong> Stock entre {formData.stockMinimo ?? 10} y {formData.stockMaximo || '∞'}</li>
                        {formData.stockMaximo && <li>🟣 <strong>EXCESO:</strong> Stock ≥ {formData.stockMaximo} unidades</li>}
                      </ul>
                    </div>
                  </div>
                </div>
