...
const PackagesPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    nombre: '',
    descripcion: '',
    productos: [] as { productId: string; cantidad: number }[],
    descuento: 0
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newPackage: Package = {
      ...
      fechaCreacion: editingPackage?.fechaCreacion || new Date()
    };

    if (editingPackage) {
      setPackages(prev => prev.map(pkg => pkg.id === editingPackage.id ? newPackage : pkg));
    } else {
                ...
                {formData.productos.map((item, index) => (
                  <div key={index} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                    ...
                    <input
                      ...
                      className="w-20 rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
                      placeholder="Cant."
                      required
                    />
        ...
        <div className="text-center py-12">
          ...
          {!searchTerm && (
            <div className="mt-6">
              <button
                onClick={handleCreatePackage}
                className="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 transition-colors"
              >
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                </svg>
