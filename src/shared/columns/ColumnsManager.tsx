...
type DensityValue = 'comfortable' | 'intermediate' | 'compact';

export interface ColumnsManagerColumn {
  id: string;
  label: string;
  visible: boolean;
...
interface ColumnsManagerProps {
  columns: ColumnsManagerColumn[];
  onToggleColumn: (columnId: string) => void;
  onResetColumns: () => void;
  onSelectAllColumns: () => void;
  onReorderColumns: (sourceId: string, targetId: string) => void;
  densityControls?: DensityControlsProps;
  buttonLabel?: string;
  title?: string;
}
...
const SortableColumnRow: React.FC<SortableColumnRowProps> = ({ column, onToggle }) => {
  ...
  const checkboxId = `columns-manager-${column.id}`;
...
export const ColumnsManager: React.FC<ColumnsManagerProps> = ({
  ...
  return (
    <div className="relative" data-column-manager="true">
      <button
        type="button"
        ref={triggerRef}
                ...
                <div className="flex gap-2">
                  {(['comfortable', 'intermediate', 'compact'] as const).map((value) => (
                    <button
                      key={value}
                      type="button"
                      onClick={() => densityControls.onChange(value)}
                      className={`flex-1 px-3 py-2 text-xs rounded-lg transition-colors ${
                        densityControls.value === value
