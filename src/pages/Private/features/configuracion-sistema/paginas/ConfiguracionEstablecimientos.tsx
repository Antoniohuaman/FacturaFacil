...
export function EstablecimientosConfiguration() {
  ...
  const manejarEnvio = (e: React.FormEvent) => {
    ...
    try {
      let updated: Establecimiento[];
      if (editingEstablecimientoId) {
        updated = Establecimientos.map(est => est.id === editingEstablecimientoId ? { ...est, ...datosFormulario, actualizadoElEstablecimiento: new Date() } : est);
        showToast('success', 'Establecimiento actualizado correctamente');
      } else {
        const nuevo: Establecimiento = { id: generateUniqueId(), ...datosFormulario, coordinates: undefined, businessHours: {}, sunatConfiguration: { isRegistered: false }, posConfiguration: undefined, inventoryConfiguration: { managesInventory: false, isalmacen: false, allowNegativeStock: false, autoTransferStock: false }, financialConfiguration: { handlesCash: true, defaultCurrencyId: '', acceptedCurrencies: [], defaultTaxId: '', bankAccounts: [] }, estadoEstablecimiento: 'ACTIVE', estaActivoEstablecimiento: true, isMainEstablecimiento: false, creadoElEstablecimiento: new Date(), actualizadoElEstablecimiento: new Date() };
        updated = [...Establecimientos, nuevo];
        showToast('success', 'Establecimiento creado correctamente');
      }
      dispatch({ type: 'SET_EstablecimientoS', payload: updated });
      handleCancel();
    } catch (err) {
      console.error('Error saving Establecimiento:', err);
        ...
        <div className="flex-1 overflow-auto">
          <div className="max-w-4xl mx-auto p-6 space-y-6">
            <div className="fixed top-4 right-4 z-50 space-y-2">
              {toasts.map(t => (
                <div key={t.id} className={`flex items-center gap-3 min-w-[300px] px-4 py-3 rounded-lg shadow-lg transform transition-all duration-300 animate-slide-in ${t.type === 'success' ? 'bg-green-50 border border-green-200 text-green-800' : t.type === 'error' ? 'bg-red-50 border border-red-200 text-red-800' : 'bg-yellow-50 border border-yellow-200 text-yellow-800'}`}>
                  {t.type === 'success' && <CheckCircle className="w-5 h-5 text-green-600" />}
                  {t.type === 'error' && <XCircle className="w-5 h-5 text-red-600" />}
                  {t.type === 'warning' && <AlertCircle className="w-5 h-5 text-yellow-600" />}
