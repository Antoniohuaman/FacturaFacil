import { CheckCircle2 } from 'lucide-react';
import { useRef, useState } from 'react';
import { MAXIMO_IDEA_RETROALIMENTACION } from '../constantes';
import { useFeedback } from '../../feedback/useFeedback';
import { CampoComentarioBreve } from './CampoComentarioBreve';
import { useEstadoEnvioLocal } from '../hooks/useEstadoEnvioLocal';
import { useRetroalimentacion } from '../hooks/useRetroalimentacion';
import { registrarInicioIdea } from '../integracionAnalitica';
import { normalizarTextoBreve } from '../reglasFrecuencia';

export function BuzonIdeas() {
  const { enviarIdea, cerrarPanel } = useRetroalimentacion();
  const feedback = useFeedback();
  const [contenido, setContenido] = useState('');
  const { enviado, enviando, ejecutarEnvioLocal, reiniciarEstadoEnvio } = useEstadoEnvioLocal();
  const registroInicioRef = useRef(false);

  const contenidoNormalizado = normalizarTextoBreve(contenido);
  const puedeEnviar = contenidoNormalizado.length > 0 && !enviando;

  const actualizarContenido = (valor: string) => {
    setContenido(valor);

    if (enviado) {
      reiniciarEstadoEnvio();
    }

    if (!registroInicioRef.current && normalizarTextoBreve(valor).length > 0) {
      registroInicioRef.current = true;
      registrarInicioIdea();
    }
  };

  const enviar = async () => {
    const ok = await ejecutarEnvioLocal(() => enviarIdea({ contenido: contenidoNormalizado }));

    if (!ok) {
      feedback.warning('Escribe una idea breve y concreta antes de enviarla.', 'Buzón de ideas');
      return;
    }

    setContenido('');
    registroInicioRef.current = false;
  };

  if (enviado) {
    return (
      <div className="space-y-5 px-4 py-5">
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-4 dark:border-emerald-900/60 dark:bg-emerald-950/30">
          <div className="flex items-start gap-3">
            <CheckCircle2 className="mt-0.5 h-5 w-5 text-emerald-600 dark:text-emerald-400" />
            <div className="space-y-1">
              <h3 className="text-sm font-semibold text-emerald-900 dark:text-emerald-200">Gracias por compartir tu idea</h3>
            </div>
          </div>
        </div>

        <div className="flex flex-wrap justify-end gap-2">
          <button
            type="button"
            onClick={reiniciarEstadoEnvio}
            className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-slate-300 hover:bg-slate-50 dark:border-gray-700 dark:bg-gray-900 dark:text-slate-200 dark:hover:border-gray-600 dark:hover:bg-gray-800"
          >
            Compartir otra idea
          </button>
          <button
            type="button"
            onClick={cerrarPanel}
            className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white"
          >
            Cerrar
          </button>
        </div>
      </div>
    );
  }

  return (
    <div className="space-y-5 px-4 py-4">
      <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Cuéntanos tu idea</h3>

      <CampoComentarioBreve
        etiqueta="Tu idea"
        valor={contenido}
        onChange={actualizarContenido}
        placeholder="Ejemplo: me ayudaría tener un resumen más claro al cerrar la caja del día."
        maximoCaracteres={MAXIMO_IDEA_RETROALIMENTACION}
        filas={5}
        disabled={enviando}
      />

      <div className="flex justify-end">
        <button
          type="button"
          onClick={enviar}
          disabled={!puedeEnviar}
          className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-300 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white dark:disabled:bg-slate-700 dark:disabled:text-slate-300"
        >
          {enviando ? 'Enviando...' : 'Enviar idea'}
        </button>
      </div>
    </div>
  );
}