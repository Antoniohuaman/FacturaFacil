  ...
  const handleCrearComprobante = async (paymentPayload?: PaymentCollectionPayload): Promise<boolean> => {
    ...
    try {
      const success = await createComprobante({
        ...
        clientDocType: clienteDraftData?.tipoDocumentoCodigo,
        clientId: clienteSeleccionadoState?.id != null ? String(clienteSeleccionadoState.id) : undefined,
        clientPriceProfileId: clienteSeleccionadoState?.priceProfileId,
        fechaEmision,
        email: clienteDraftData?.email,
        address: clienteDraftData?.direccion,
