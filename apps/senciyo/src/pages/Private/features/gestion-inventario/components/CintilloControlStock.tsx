...
interface Props {
  onConfigurar: () => void;
  /** `true` cuando Inventario ya se configuró alguna vez y hoy está apagado (§28 de la corrección UX final 2026-08-07) — cambia el texto y la acción de "configurar" a "reactivar", sin abrir una implementación distinta. */
  yaConfiguradoAntes: boolean;
}

export default function CintilloControlStock({ onConfigurar, yaConfiguradoAntes }: Props) {
  return (
    <div className="mx-6 mt-3 flex flex-col sm:flex-row sm:items-center gap-3 px-5 py-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-xl">
      <div className="flex items-start gap-3 flex-1">
        ...
        <div>
          <p className="text-sm font-semibold text-blue-900 dark:text-blue-200">
            {yaConfiguradoAntes ? 'Tu inventario está inactivo' : 'Configura tu inventario'}
          </p>
          <p className="text-xs text-blue-700 dark:text-blue-400 mt-0.5 leading-relaxed">
            {yaConfiguradoAntes
              ? 'Tu configuración anterior se conserva — puedes reactivarlo cuando quieras.'
      ...
      <div className="flex-shrink-0 sm:ml-4">
        ...
        >
          {yaConfiguradoAntes ? 'Activar inventario' : 'Configurar inventario'}
        </button>
      </div>
    </div>
  );
