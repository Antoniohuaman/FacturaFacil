...
export function PaginaLeccionesAprendidas() {
  ...
  const nombresAuditoria = useMemo(() => new Map((referencias?.auditorias ?? []).map((auditoria) => [auditoria.id, `${auditoria.tipo_auditoria_codigo} · ${normalizarFechaPortal(auditoria.fecha_auditoria)}`])), [referencias])
  ...
  return (
    <section className="mx-auto flex w-full max-w-7xl flex-col gap-5">
      ...
      <div className="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 md:grid-cols-4 dark:border-slate-800 dark:bg-slate-900">
        ...
        <select value={filtroAuditoria} onChange={(evento) => { setFiltroAuditoria(evento.target.value); paginacion.setPaginaActual(1) }} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none dark:border-slate-700 dark:bg-slate-800"><option value="todos">Auditoría: todas</option>{(referencias?.auditorias ?? []).map((auditoria) => <option key={auditoria.id} value={auditoria.id}>{auditoria.tipo_auditoria_codigo} · {normalizarFechaPortal(auditoria.fecha_auditoria)}</option>)}</select>
      ...
      <ModalPortal abierto={modalAbierto} titulo={modoModal === 'crear' ? 'Nueva lección aprendida' : modoModal === 'editar' ? `Editar ${leccionActiva?.codigo ?? 'lección'}` : `Detalle ${leccionActiva?.codigo ?? 'lección'}`} alCerrar={() => setModalAbierto(false)}>
        <form className="space-y-4" onSubmit={formulario.handleSubmit(async (valores) => { try { if (modoModal === 'ver') { setModalAbierto(false); return } if (modoModal === 'crear') await crearLeccionAprendidaPm(valores); if (modoModal === 'editar' && leccionActiva) await editarLeccionAprendidaPm(leccionActiva.id, valores); setModalAbierto(false); await cargar() } catch (errorInterno) { setError(errorInterno instanceof Error ? errorInterno.message : 'No se pudo guardar la lección aprendida') } })}>
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <label className="space-y-1 text-sm md:col-span-2"><span>Auditoría</span><select {...formulario.register('auditoria_id')} disabled={modoModal === 'ver'} className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 dark:border-slate-700 dark:bg-slate-800"><option value="">Sin auditoría</option>{(referencias?.auditorias ?? []).map((auditoria) => <option key={auditoria.id} value={auditoria.id}>{auditoria.tipo_auditoria_codigo} · {normalizarFechaPortal(auditoria.fecha_auditoria)}</option>)}</select></label>
